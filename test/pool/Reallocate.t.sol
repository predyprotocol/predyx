...
contract TestReallocate is TestPool {
    ...
    function testReallocateSucceeds() public {
        predyPool.reallocate(1, settlement.getSettlementParams(filler, address(currency1), address(currency0), 1e4));
        ...
        tradeMarket.trade(
            tradeParams, settlement.getSettlementParams(filler, address(currency1), address(currency0), 1e4)
        );

        _movePrice(true, 5 * 1e16);

        predyPool.reallocate(1, settlement.getSettlementParams(filler, address(currency1), address(currency0), 14000));
    }
