...
import "../../../src/markets/gamma/GammaDutchOrderValidator.sol";

contract GammaDutchOrderValidatorTest is Test {
    GammaDutchOrderValidator dutchOrderValidator;

    function setUp() public {
        dutchOrderValidator = new GammaDutchOrderValidator();
    }

    function testValidateEndTimeBeforeStartTime() public {
        GammaDutchOrderValidationData memory dutchOrderValidationData;
    ...
    function testValidatePriceGreaterThanLimit(int256 tradeAmount) public {
        ...
        if (tradeAmount > 0) {
            vm.expectRevert(GammaDutchOrderValidator.PriceGreaterThanLimit.selector);
        }
        dutchOrderValidator.validate(gammaOrder, tradeResult);
    }
    ...
    function testValidatePriceLessThanLimit(int256 tradeAmount) public {
        ...
        if (tradeAmount < 0) {
            vm.expectRevert(GammaDutchOrderValidator.PriceLessThanLimit.selector);
        }
        dutchOrderValidator.validate(gammaOrder, tradeResult);
    }
}
